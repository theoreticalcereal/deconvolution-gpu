...
def reference_petakit_rl(observed, psf, n_iters, background=0.0):
...
class PsfToOtfTests(unittest.TestCase):
    def test_psf_to_otf_matches_petakit_post_padding_and_center_roll(self):
        from petakit_rl import psf_to_otf
    ...
    def test_fit_psf_to_shape_uses_petakit_center_crop(self):
        from petakit_rl import fit_psf_to_shape
...
class PetakitSimplifiedRlTests(unittest.TestCase):
    def setUp(self):
        rng = np.random.default_rng(42)
        self.observed = rng.uniform(10, 500, size=(5, 7, 9)).astype(np.float32)
        self.psf = asymmetric_psf()

    def test_iterations_match_independent_petakit_translation(self):
        from petakit_rl import petakit_simplified_rl

        for n_iters in (1, 2, 5):
            with self.subTest(n_iters=n_iters):
                expected = reference_petakit_rl(
                    self.observed, self.psf, n_iters, background=7.0
                )
                actual = petakit_simplified_rl(
                    self.observed, self.psf, n_iters, background=7.0
                )
    ...
    def test_restoration_is_nonnegative_finite_and_preserves_flux(self):
        ...
        actual = petakit_simplified_rl(self.observed, self.psf, 5)
...
class CupyIntegrationTests(unittest.TestCase):
    ...
    def test_cupy_uint16_wrapper_matches_numpy_core(self):
        ...
        from petakit_rl import petakit_simplified_rl, restore_uint16_cupy

        rng = np.random.default_rng(7)
        observed = rng.integers(0, 2000, size=(5, 7, 9), dtype=np.uint16)
        psf = asymmetric_psf()
        expected_float = petakit_simplified_rl(
    ...
    def test_cupy_wrapper_transfers_once_and_returns_uint16_host_array(self):
        ...
        expected = np.floor(
            np.clip(
                petakit_simplified_rl(observed, psf, 2, background=3),
                0,
                65535,
            ) + 0.5
        ).astype(np.uint16)
